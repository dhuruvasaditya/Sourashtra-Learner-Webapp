...
web/phrases.json     copy of the dataset, served to the app
web/audio/0042.mp3   one file per phrase
```

Every save writes all of these, so there is no export step. Committing and pushing
`web/` is what publishes new phrases to the learners.

## The learning app

`web/` is the whole app — one HTML file, no build step, no backend.
...
branch, folder `/web`. Anything you commit under `web/` goes live.
